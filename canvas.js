...
class Bug {
    ...
    update() {
        ...
        if (this.mouthAngle >= this.mouthMaxAngle || this.mouthAngle <= 0) {
            this.mouthDirection *= -1; // Change direction when reaching max angle or 0
        }
    ...
    cleanImpurities(molecules) {
        // Define a threshold distance within which impurities will be cleaned
        const cleaningRange = 50;

        // Loop through all molecules
        molecules.forEach(molecule => {
            // Check if the molecule is not blue (assuming impurities are not blue)
            if (molecule.color !== 'blue') {
                // Calculate distance between bug and molecule
                const distance = Math.sqrt(Math.pow(this.x - molecule.x, 2) + Math.pow(this.y - molecule.y, 2));
                // If molecule is within the cleaning range, gradually reduce its radius
                if (distance < cleaningRange) {
                    // Reduce the radius of the impurity molecule gradually
                    if (molecule.radius > 0.05) {
                        molecule.radius -= 0.05;
                    } else {
                        // If radius is below a certain threshold, remove the molecule from the array
...
function startCleaningProcess() {
    for (let i = 0; i < 5; i++) {
        let radius = 20;
        let genX = randomIntFromRange(radius, canvas.width - radius);
        let genY = randomIntFromRange(radius, canvas.height - radius);
        let velocity = {
            x: Math.random() * randomIntFromRange(3, 8),
            y: Math.random() * randomIntFromRange(3, 8)
        }
        // Create bug objects instead of charcoal balls
        charcoalBallsArray.push(new Bug(genX, genY, velocity, radius, 'brown'));
    }
}
...
function animate() {
    ...
    charcoalBallsArray.forEach(ball => {
        ball.update(); // Call update method instead of updateCharcoalBall
    })
    charcoalBallsArray.forEach(bug => {
        bug.cleanImpurities(moleculeArray);
        bug.update();
    });
